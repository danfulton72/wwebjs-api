"""Tests for the WhatsApp HA Home Assistant config flow."""
...
from custom_components.whatsapp.api import WWebJSApiError
...
async def test_user_config_flow_creates_entry(hass) -> None:
    ...
    with patch(
        "custom_components.whatsapp.config_flow.WWebJSApiClient.async_get_sessions",
...
async def test_configuration_yaml_is_imported(hass) -> None:
    """Test configuration.yaml is converted into a config entry."""
    with patch(
        "custom_components.whatsapp.config_flow.WWebJSApiClient.async_get_sessions",
        new=AsyncMock(return_value=[]),
    ):
